...
class ListDetailTransition extends StatefulWidget {
  ...
  @override 
  State<ListDetailTransition> createState() => _ListDetailTransition();
}

class _ListDetailTransition extends State<ListDetailTransition> {
  ...
  void didChangeDependencies() {
    ...
    if(nextFlexFactor == currentFlexFactor) return;
    ...
    if(currentFlexFactor == 0) {
      ...
      final TweenSequence<double> sequence = TweenSequence([
        if(sizeAnimation.value > 0) ...[
          TweenSequenceItem(tween: Tween(begin: 0, end: widthAnimation.value), weight: sizeAnimation.value) 
        ],
        if(sizeAnimation.value < 1) ...[
          TweenSequenceItem(tween: Tween(begin: widthAnimation.value, end: nextFlexFactor), weight: 1-sizeAnimation.value)
        ]
      ]);
